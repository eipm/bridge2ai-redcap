<p align="center">
    <img src="../../../../../images/main_logo_black.svg#gh-light-mode-only" width="200" alt="B2Ai Voice Logo">
    <img src="../../../../../images/main_logo_white.svg#gh-dark-mode-only" width="200" alt="B2Ai Voice Logo"><br>
    Voice as a Biomarker of Health
</p>

[![Back to Main Document](https://img.shields.io/badge/back%20to%20main%20document-8A2BE2)](../../../../../README.md)

[mic]: https://custom-icon-badges.demolab.com/badge/Presione_para_grabar-purple.svg?logo=mic&logoSource=feather
[word]: https://custom-icon-badges.demolab.com/badge/Word-orange.svg?logo=refresh-cw&logoSource=feather
[recording_1]: https://img.shields.io/badge/Recording%201-white

# Vocabulario Productivo

![recording_1][recording_1]

[![word][word]](Productive%20Vocabulary%20Words-es-419.md)

No conozco esta palabra, siguiente

A continuación, se le proporcionará una serie de palabras. Puede que conozca o no la palabra.

Si conoce la palabra, por favor proporcione una definición de la misma. Después de definir la palabra, presione **"Siguiente Palabra"**.

Si NO conoce la palabra, presione **"No conozco esta palabra, siguiente"** y se le proporcionará una nueva palabra.

Una vez que se hayan definido 6 palabras, aparecerá el botón “Tarea Completada”.

![mic][mic]
