<p align="center">
    <img src="../../../../../images/main_logo_black.svg#gh-light-mode-only" width="200" alt="B2Ai Voice Logo">
    <img src="../../../../../images/main_logo_white.svg#gh-dark-mode-only" width="200" alt="B2Ai Voice Logo"><br>
    Voice as a Biomarker of Health
</p>

[![Back to Main Document](https://img.shields.io/badge/back%20to%20main%20document-8A2BE2)](../../../../../README.md)

[mic]: https://custom-icon-badges.demolab.com/badge/Press_to_Record-purple.svg?logo=mic&logoSource=feather

[recording_1]: https://custom-icon-badges.demolab.com/badge/Recording%201-Picture%20description-8A2BE2.svg?logo=b2ai_voice&logoColor=white
[recording_2]: https://custom-icon-badges.demolab.com/badge/Recording%201-Picture%20description--option1-8A2BE2.svg?logo=b2ai_voice&logoColor=white
[recording_3]: https://custom-icon-badges.demolab.com/badge/Recording%201-Picture%20description--option2-8A2BE2.svg?logo=b2ai_voice&logoColor=white
[picture_descriptions]: https://custom-icon-badges.demolab.com/badge/Picture_Description_1_|_Picture_Description_2-darkgreen.svg?logo=image&logoSource=feather
[picture_description_1]:  https://custom-icon-badges.demolab.com/badge/Picture_Description_1-darkgreen.svg?logo=image&logoSource=feather
[picture_description_2]:  https://custom-icon-badges.demolab.com/badge/Picture_Description_2-darkgreen.svg?logo=image&logoSource=feather

# Picture Description

> **Important Note:** When the 2nd picture was introduced in the protocol, the 2nd picture became `picture description-option2`. `Picture description` vs `Picture description-option1` are recordings of the same picture description task, but the former was recorded before the 2nd picture was introduced and the latter was recorded after the 2nd picture was introduced.

This task helps us analyze features in your voice.

![picture_descriptions][picture_descriptions]

![mic][mic]

---

![recording_1][recording_1] or ![recording_2][recording_2]

![picture_description_1][picture_description_1]

Tell me everything you see going on in this picture.

![Alt text](PictureDescriptionTaskPicture1.png)

---

![recording_3][recording_3]

![picture_description_2][picture_description_2]

Describe everything that is happening in the picture (as though describing it for the blind), trying to use complete sentences.

![Alt text](PictureDescriptionTaskPicture2.jpg)